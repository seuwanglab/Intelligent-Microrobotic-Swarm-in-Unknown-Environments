...
for step in tqdm(all_steps, desc="Processing steps"):
    ...
    for head_idx, head_df in enumerate(head_dfs):
        ...
        for i in range(available_steps):
            ...
            weight = float(current_attention[key_col]) / 4.0
            ...
            for obst_idx in range(1, 6):
                ...
                if obst_shape <= 0.5:
                    ...
                    X_obs, Y_obs = np.meshgrid(x_range, y_range)
                    dx_obs = X_obs - obst_x
                    dy_obs = Y_obs - obst_y
                    mask_obs = ((dx_obs / rx) ** 2 + (dy_obs / ry) ** 2) <= 1
                    obst_grid[y_min:y_max + 1, x_min:x_max + 1] += weight * mask_obs
    ...
    save_path = os.path.join(output_dir, f'step{step:04d}.png')
